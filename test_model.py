...
def getLargestCC(segmentation, paired):
    ...
    # For paired organs get the first two CC
        ...
        assert( labels.max() > 1 ) # assume at least 2 CC
        largestCC = \
...
parser.add_argument('-path', '--path_to_model_folder', type=str, required=True,
...
## Load net
# Init
model = unet_3D(input_shape=(patch_dim[0], 
                             patch_dim[1], 
                             patch_dim[2], 
                             n_input_channels), 
                number_of_pooling=2, 
                ...
                lr=1e-3)
# Pull weights
model.load_weights(os.path.join(args.path_to_model_folder, args.model_name))
...
###############################################################################
## Paths
path_to_results = os.path.join(args.path_to_model_folder, 
...
for ID in list_IDs:
    ...
    input_ct_standardized = np.zeros((1, input_shape[0], input_shape[1], input_shape[2], 1))
    ...
    # Init
    prediction_mask = np.zeros((input_shape[0], 
                                 input_shape[1], 
                                 input_shape[2]))
    overlap_map = np.zeros((input_shape[0], 
                                 input_shape[1], 
                                 input_shape[2]))
    ...
    while h + patch_dim[2] < input_shape[2]:
        ...
        for l in range(0, 384, 128):
            ...
            for w in range(0, 384, 128):
                ...
                # Predict
                prediction_patch = \
                    model.predict(input_ct_standardized[:, 
                                                        l:l+patch_dim[0], 
                                                        w:w+patch_dim[1], 
                                                        h:h+patch_dim[2],
                                                        :])
    ...
    #print(prediction_mask.dtype)

    ###########################################################################
    ### COMPUTE SCORES
    ###########################################################################
    # Compute    
    intersection = np.sum(groundtruth_mask * prediction_mask, axis=(0,1,2))
    summation = np.sum(groundtruth_mask + prediction_mask, axis=(0,1,2))
