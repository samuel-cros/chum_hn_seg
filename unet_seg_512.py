...
def conv_block(output_size, previous_layer, n_convolutions, activation, 
                kernel_initializer, batch_norm, dropout):
    # Convolve
    block = previous_layer
    for i in range(n_convolutions):
        block = Conv3D(output_size, kernel_size = 3, padding = 'same', 
...
def up_conv_block(output_size, previous_layer, skip_connections_layer, 
                    n_convolutions, activation, kernel_initializer, 
                    batch_norm, dropout):
    block = previous_layer
    # Deconvolve
    block = Conv3DTranspose(output_size, kernel_size = 3, padding = 'same', 
                            kernel_initializer = kernel_initializer)\
                                (UpSampling3D(size = (2,2,2))(block))
    # BN
    if batch_norm:
        block = BatchNormalization()(block)
    # Activate
...
def bce_dice_loss(y_true, y_pred, bce_weight=1e-3, dice_weight=1):
    ...
    return bce_weight * losses.binary_crossentropy(y_true_f, y_pred_f) \
        - dice_weight * dice_coefficient(y_true, y_pred)

# Current
current_loss = dice_coefficient_loss
...
def unet(input_size, n_output_channels, dropout_value, 
        n_convolutions_per_block, optim, lr, pretrained_weights = None):
    ...
    conv16, pool16 = conv_block(16, inputs, n_convolutions_per_block, 
                                inner_activation, kernel_value, batch_norm, 
                                dropout_value)
    # x32 layers going down
    conv32, pool32 = conv_block(32, pool16, n_convolutions_per_block, 
                                inner_activation, kernel_value, batch_norm, 
                                dropout_value)
    ...
    upconv256 = up_conv_block(256, conv512, conv256, n_convolutions_per_block, 
                                inner_activation, kernel_value, batch_norm, 
                                dropout_value)
    # x128 layers going up
    upconv128 = up_conv_block(128, upconv256, conv128, 
                                n_convolutions_per_block, inner_activation, 
                                kernel_value, batch_norm, dropout_value)
    # x64 layers going up
    upconv64 = up_conv_block(64, upconv128, conv64, n_convolutions_per_block, 
                            inner_activation, kernel_value, batch_norm, 
                            dropout_value)
    # x32 layers going up
    upconv32 = up_conv_block(32, upconv64, conv32, n_convolutions_per_block, 
                            inner_activation, kernel_value, batch_norm, 
                            dropout_value)
    # x16 layers going up
    upconv16 = up_conv_block(16, upconv32, conv16, n_convolutions_per_block, 
                                inner_activation, kernel_value, batch_norm, 
                                dropout_value)
    # Output
    convFIN = Conv3D(n_output_channels, kernel_size = 1, 
                        activation = final_activation)(upconv16)
