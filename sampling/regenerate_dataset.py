...
# Parameters
dilation_spacing = (1, 1, 4) # ratio based on 256*256*64 patches
dilation_radius = 0.8*256//2 # 80% of patch_dim[0]//2
...
h5_file = h5py.File(os.path.join('..', '..', 'data', 'CHUM', 'h5_v3', 
                                 'regenerated_dataset.h5'), 'w')
...
for ID in IDs:
    ...
    # Add CT
    h5_file.create_dataset(ID + '/ct', data = data["scans"], 
                           compression="gzip")
    ...
    for channel_name in data["masks"].attrs["names"]:
        if channel_name not in extra_volumes:
            ...
            # Add mask
            h5_file.create_dataset(ID + '/mask/' + channel_name, 
                                    data=data["masks"][h5_index], 
                                    compression="gzip")

            # Add dilated mask
            dilated_data = binary_dilation(data["masks"][h5_index], 
                                           (1,1,1), 
                                           dilation_radius)
            h5_file.create_dataset(ID + '/dilated_mask/' + channel_name, 
                                   data = dilated_data, 
                                   compression="gzip")
