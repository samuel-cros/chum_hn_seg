...
if len(sys.argv) >=7:
    ...
    print("python test_multi_params_for_avg.py path_to_model_dir model_name model_depth mode_for_H kind_of_oars train_validation_or_test_or_manual list_IDs_if_manual")
...
# Saving results
path_to_results = os.path.join(path_to_model_dir, 'results_slices_' + train_validation_or_test_or_manual)
...
for ID in list_IDs:
    ...
    if mode_for_H == 'centered':
        H = ct.shape[2]//2 - patch_dim[2]//2 # centered for lower organs within the volume
    elif mode_for_H == 'up':
    ...
    #'''
    # CSV
    csv_file = open(os.path.join(path_to_predicted_volume_for_ID, ID + '_dice_scores.csv'), 'w', newline='')
    ...
    for h in range(0, prediction.shape[2], 4):
        ...
        ct_fig.savefig(os.path.join(path_to_expected_volume_for_ID, 'ct_slice_' + str(H+h)))
        plt.close()
        ...
        plt.imshow(np.ma.masked_where(groundtruth[:, :, h] == 0, groundtruth[:, :, h] == 0).T * dict_oars[list_oars[0]], norm=oar_norm, cmap=oar_cmap)
        ax = plt.gca()
        ax.set_facecolor('black')
        ax.invert_xaxis()
        gt_fig.savefig(os.path.join(path_to_expected_volume_for_ID, 'gt_slice_' + str(H+h)))
        plt.close()

        #print('PRED..')
        # Produce PRED
        pred_fig = plt.figure(3, figsize=(8,5))
        plt.imshow(np.ma.masked_where(prediction[:, :, h, 0] == 0, prediction[:, :, h, 0]).T * dict_oars[list_oars[0]], norm=oar_norm, cmap=oar_cmap)
        ax = plt.gca()
        ax.set_facecolor('black')
        ax.invert_xaxis()
        pred_fig.savefig(os.path.join(path_to_predicted_volume_for_ID, 'pred_slice_' + str(H+h)))
        plt.close()
