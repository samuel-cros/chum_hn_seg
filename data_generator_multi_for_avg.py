...
chum_directory = os.path.join(working_directory_path, "..", "data", "CHUM", 
                                "h5_v2")
...
def generate_random_patch(self, input_path):
    ...
    if self.cropping == 'all':
        L = random.randint(364 - self.patch_dim[0], 153)
        ...
        L = random.randint(364 - self.patch_dim[0], 174)
        W = random.randint(1, 395 - self.patch_dim[1])
        H = random.randint(0, shape_scans[2] - self.patch_dim[2])

    elif self.cropping == 'up': # oeil, parotide, etc
        L = random.randint(363 - self.patch_dim[0], 153)
        W = random.randint(66, 376 - self.patch_dim[1])
        temp = shape_scans[2] - self.patch_dim[2]
        if temp >= 51:
            H = random.randint(51, min(shape_scans[2], 334) \
                - self.patch_dim[2])
        else:
            H = temp

    elif self.cropping == 'parotides':
        min_3D_d, max_3D_d = self.min_locations_dict['parotide d'], \
                                self.max_locations_dict['parotide d']
        min_3D_g, max_3D_g = self.min_locations_dict['parotide g'], \
                                self.max_locations_dict['parotide g']
        ...
        if (max_3D[0] - min_3D[0] >= self.patch_dim[0]):
            ...
            if max_3D[0] >= self.patch_dim[0]:
                L = random.randint(max_3D[0] - self.patch_dim[0], 
                                    min(min_3D[0], self.patch_dim[0]))
            else:
        ...
        if (max_3D[1] - min_3D[1] >= self.patch_dim[1]):
            ...
            if max_3D[1] >= self.patch_dim[1]:
                W = random.randint(max_3D[1] - self.patch_dim[1], 
                                    min(min_3D[1], self.patch_dim[1]))
            else:
                W = random.randint(0, min(min_3D[1], self.patch_dim[1]))

        # H (shape_scans[2] is NOT constant at around 200-400 with some 
        # particular cases at around 100)
        ...
        min_3D_d, max_3D_d = self.min_locations_dict['oeil d'], \
                                self.max_locations_dict['oeil d']
        min_3D_g, max_3D_g = self.min_locations_dict['oeil g'], \
                                self.max_locations_dict['oeil g']
        ...
        min_3D_d, max_3D_d = self.min_locations_dict['sous-max d'], \
                                self.max_locations_dict['sous-max d']
        min_3D_g, max_3D_g = self.min_locations_dict['sous-max g'], \
                                self.max_locations_dict['sous-max g']
        ...
        min_3D_d, max_3D_d = self.min_locations_dict['oreille int d'], \
                                self.max_locations_dict['oreille int d']
        min_3D_g, max_3D_g = self.min_locations_dict['oreille int g'], \
                                self.max_locations_dict['oreille int g']
        ...
        # OAR specific
        min_3D, max_3D = self.min_locations_dict[self.cropping], \
                            self.max_locations_dict[self.cropping]
    ...
    new_output = np.zeros((self.patch_dim[0], self.patch_dim[1], 
                            self.patch_dim[2], self.n_output_channels)) #
    ...
    for channel_name in h5_file["masks"].attrs["names"]:
        if channel_name not in tumor_volumes and \
            channel_name in self.list_oars:
            new_output[:, :, :, 0] += \
                h5_file["masks"][h5_index, 
                                    L:L+self.patch_dim[0], 
                                    W:W+self.patch_dim[1], 
                                    H:H+self.patch_dim[2]]
        h5_index += 1
    ...
    new_input = np.zeros((self.patch_dim[0], self.patch_dim[1], 
                            self.patch_dim[2], self.n_input_channels))

    # Fill the CT channel
    new_input[:, :, :, 0] = h5_file["scans"][L:L+self.patch_dim[0], 
                                            W:W+self.patch_dim[1], 
                                            H:H+self.patch_dim[2]]
    ...
    if self.augmentation: # TOREDO
        ...
        # Affine transform
        random_angle = \
            np.pi/random.choice([neg for neg in range(-32, -15, 1)] + \
                                [pos for pos in range(16, 33, 1)])
        random_shear = \
            np.array([[1, random.choice([0.0, 0.1, 0.2]+[-0.1,-0.2]), 0], 
                        [0, 1, 0], [0, 0, 1]])
        ...
        #ct *= random_brighting
        affine = \
            gryds.AffineTransformation(ndim=3, angles=[0, 0, random_angle], 
                                        center=[0.5,0.5,0.5], 
                                        shear_matrix=random_shear)
        ...
        new_augmented_output = np.zeros((self.patch_dim[0], 
                                            self.patch_dim[1], 
                                            self.patch_dim[2], 
                                            self.n_output_channels))
        with concurrent.futures.ProcessPoolExecutor() as executor:
            results = [executor.submit(interpolate, new_output[:, :, :, k], 
            affine) for k in range(self.n_output_channels)] # Out of Memory
...
class DataGenerator(keras.utils.Sequence):
    ...
    def __init__(self, train_or_validation, list_IDs, list_oars, patch_dim, 
                    batch_size, n_input_channels, n_output_channels, cropping, 
                    min_locations_dict, max_locations_dict, shuffle=True, 
                    augmentation=False):
        'Initialization'
